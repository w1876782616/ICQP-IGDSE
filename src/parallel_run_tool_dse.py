...
import shutil
from src.utils import get_ts, create_dir_if_not_exists, get_src_path
...
class Saver:
    ...
    def __init__(self, kernel):
        self.logdir = join(get_src_path(), 'logs', 'dse-results', f'{kernel}_{get_ts()}')
...
def kernel_parser() -> argparse.Namespace:
    ...
    parser_run.add_argument('--kernel', required=True, action='store', help='Kernel Name')
    parser_run.add_argument('--benchmark', required=True, action='store', help='Benchmark Name')
    parser_run.add_argument('--root-dir', required=True, action='store', default='.', help='GNN Root Directory')
    parser_run.add_argument('--redis-port', required=True, action='store', default='6379', help='The port number for redis database')
    return parser_run.parse_args()

def persist(database, db_file_path) -> bool:
    dump_db = {key: database.hget(0, key) for key in database.hgetall(0)}
