...
from src.utils import get_root_path, MLP, print_stats
...
from src.utils import save_fig, create_dir_if_not_exists
import pickle
...
def tsne(df, feat_cols, n_class, obj=None):
    ...
    sns_plot = sns.scatterplot(x='tsne-2d-one', y='tsne-2d-two', hue='y', palette=cmap, data=df, legend='full')
...
def _plot_gs(selected_gs, conf):
    ...
    info_dict = {'draw_node_size': 20, 'draw_node_label_enable': True, 'show_labels': False, 'node_label_type': 'type', 'node_label_name': 'type', 'draw_node_label_font_size': 6, 'draw_node_color_map': get_color_map(selected_gs), 'draw_edge_label_enable': False, 'draw_edge_label_font_size': 6, 'each_graph_text_list': [], 'each_graph_text_font_size': 10, 'each_graph_text_pos': [0.5, 1.05], 'top_space': 0.2, 'bottom_space': 0.05, 'hbetween_space': 0.6, 'wbetween_space': 0, 'plot_dpi': 200, 'plot_save_path_eps': '', 'plot_save_path_png': ''}
    plt_cnt = 0
    info_dict, plt_cnt = set_save_paths_for_vis(info_dict, DIR, None, 'binary-{}_{}_gs'.format(DATASET, conf.name()), plt_cnt)
...
if __name__ == '__main__':
    ...
    with open(save_path_x, 'wb') as fp:
