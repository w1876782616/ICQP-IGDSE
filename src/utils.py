...
def exec_cmd(cmd, timeout=None, exec_print=True):
    ...
    if not timeout:
        ...
        def run(cmd, timeout_sec):
            ...
            timer.cancel()
            return (proc.returncode, stdout.decode('utf-8'), stderr.decode('utf-8'), timeout_dict['value'])
        if global_exec_print and exec_print:
...
class OurTimer(object):
    ...
    def print_durations_log(self):
        ...
        for log_str, duration in self.durations_log.items():
            s = '{0}{1} : {2} ({3:.2%})'.format(log_str, ' ' * (lss - len(log_str)), format_seconds(duration), duration / tot_duration)
            rtn.append(s)
            print(s)
        print('Timer log', '*' * 50)
        self.durations_log = OrderedDict()
...
def format_seconds(seconds):
    periods = [('year', 60 * 60 * 24 * 365), ('month', 60 * 60 * 24 * 30), ('day', 60 * 60 * 24), ('hour', 60 * 60), ('min', 60), ('sec', 1)]
    if seconds <= 1:
...
def plot_points(points_dict, label, save_dir):
    ...
    for pname, points in points_dict.items():
        ...
        plt.plot(xs, ys, POINTS_MARKERS[i % len(POINTS_MARKERS)], color=POINTS_COLORS[i % len(POINTS_COLORS)], label=f'{pname}_{label}')
        i += 1
    plt.legend(loc='best')
...
def plot_points_with_subplot(points_dict_multi_target, label, save_dir, target_list):
    ...
    for idx, target in enumerate(target_list):
        ...
        for pname, points in points_dict.items():
            xs = [point[0] for point in points]
            ys = [point[1] for point in points]
            plt.plot(xs, ys, POINTS_MARKERS[i % len(POINTS_MARKERS)], color=POINTS_COLORS[i % len(POINTS_COLORS)], label=f'{pname}')
            i += 1
        plt.legend(loc='best')
        plt.title(f'{target}')
        points_dict = {}
