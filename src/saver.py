...
from src.utils import get_src_path, get_model_info_as_str, extract_config_code, plot_scatter_line, plot_dist, save_pickle, dirname
...
class Saver(object):
    ...
    def __init__(self):
        ...
        self.logdir = join(get_src_path(), 'logs', f"{FLAGS.model_tag}_{FLAGS.norm_method}_{FLAGS.task}_{FLAGS.subtask}_{FLAGS.tag}_{''.join(FLAGS.target)}_{model_str}_{get_ts()}")
    ...
    def log_scatter_mcs(self, cur_id, iter, result_d):
        ...
        for label, data_dict in result_d.items():
            for model_name, data_dict_elt in data_dict['result'].items():
                ...
                self._save_to_result_file(f'  num_iters={len(incumbent_size_list)}, runtime={runtime}, mcs={incumbent_size_list[-1]}, method={model_name}', to_print=True)
    ...
    def save_pairs_with_results(self, test_data, train_data, info):
        ...
        save({'test_data_pairs': self._shrink_space_pairs(test_data.dataset.pairs)}, p, print_msg=False)

    def save_ranking_mat(self, true_m, pred_m, info):
        p = join(self.get_log_dir(), '{}_ranking_mats'.format(info))
        save({'true_m': true_m.__dict__, 'pred_m': pred_m.__dict__}, p, print_msg=False)
