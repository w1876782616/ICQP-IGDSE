...
class EnvArgs(NamedTuple):
    ...
    def load_net(self) -> ModuleList:
        component_list = self.model_type.get_component_list(in_dim=self.env_dim, hidden_dim=self.env_dim, out_dim=self.env_dim, num_layers=self.num_layers, bias=True, edges_required=True, gin_mlp_func=self.gin_mlp_func)
        if self.dec_num_layers > 1:
            mlp_list = (self.dec_num_layers - 1) * [Linear(self.env_dim, self.env_dim), Dropout(self.dropout), self.act_type.nn()]
...
class ActionNetArgs(NamedTuple):
    ...
    def load_net(self) -> ModuleList:
        net = self.model_type.get_component_list(in_dim=self.env_dim, hidden_dim=self.hidden_dim, out_dim=2, num_layers=self.num_layers, bias=True, edges_required=False, gin_mlp_func=self.gin_mlp_func)
