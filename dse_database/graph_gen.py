        ...
        for idx_, line in enumerate(lines_source[idx_start:last_line]):
            ...
            if pragma_zone:
                ...
                if line.startswith('#pragma'):
                    ...
                    raise RuntimeError()
            elif line.startswith('#pragma') and (not 'KERNEL' in line.upper()):
                pragma_list = [line]
                pragma_zone = True
    if log:
...
def graph_generator(name, path, benchmark, generate_programl=False):
    ...
    assert for_count_llvm == for_count_source, f'the number of for loops from the LLVM code and source code do not match {for_count_llvm} in llvm vs {for_count_source} in the code'
